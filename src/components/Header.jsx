  ...
  return (
    <div className='h-[100px] w-full flex justify-end items-center border-b-[4px] border-blue-500 gap-3'>
        {navItems?.map((item,index)=>(
            <span key={index} className={`text-lg font-semibold pb-1 ${location.pathname === item.path ? "border-b-[3px] border-blue-600":""} cursor-pointer`} onClick={()=>navigate(item.path)}>{item.label}</span>
        ))}
    </div>
  )
}
